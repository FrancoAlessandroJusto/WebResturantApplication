...
class DatabaseManager:
    ...
    @staticmethod
    def execute_update(query: str, params: tuple = ()) -> int:
        """Esegue query INSERT/UPDATE/DELETE e restituisce rowcount"""
        with DatabaseManager.get_connection() as conn:
            cursor = conn.execute(query, params)
            conn.commit()
    ...
    def execute_many(query: str, params_list: list) -> int:
        """Esegue query multiple con executemany"""
        with DatabaseManager.get_connection() as conn:
...
def handle_database_errors(func: Callable) -> Callable:
    """
    Decorator per gestire automaticamente gli errori database.
    
    Centralizza error handling, logging e response format.
    """
    @wraps(func)
    def wrapper(*args, **kwargs):
        try:
            ...
            raise HTTPException(400, f"Data integrity error: {str(e)}")
...
# Import HTTPException per il decorator
try:
    from fastapi import HTTPException
except ImportError:
    # Fallback per import in altri contesti
